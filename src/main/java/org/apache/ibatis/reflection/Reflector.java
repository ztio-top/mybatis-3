...
public class Reflector {
  ...
  /**
   * 初始化 getMethods + getTypes 和 setMethods + setTypes ，通过遍历 fields 属性。实际上，它是 #addGetMethods(...) 和 #addSetMethods(...) 方法的补充，因为有些 field ，不存在对应的 setting 或 getting 方法，所以直接使用对应的 field ，而不是方法。
   * @param clazz
   */
  ...
  private void addSetField(Field field) {
    if (isValidPropertyName(field.getName())) {
      // 添加到 setMethods 中 ， 注意，此处创建的是 SetFieldInvoker 对象。详细解析，见 「4.2 SetFieldInvoker」
